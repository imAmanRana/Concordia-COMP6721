...
public class BFS {
	...
	public static final String name = "puzzleBFS";
	...
	static Queue<ElevenPuzzleState> openStates = new PriorityQueue<>(Comparator
			.comparingInt(ElevenPuzzleState::get_H_Cost).thenComparingInt(ElevenPuzzleState::getOrdinalValue));
	static Stack<ElevenPuzzleState> solutionPath = new Stack<>();

	public static void performBFS(ElevenPuzzleState ep, Heuristic heuristic) {
		// add the root node
		openStates.add(ep);
		BFS.heuristic = heuristic;
		System.out.println("\nPerforming BFS with " + heuristic + " heuristic\n");
