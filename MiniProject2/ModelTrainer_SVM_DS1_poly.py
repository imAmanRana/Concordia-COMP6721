...
def myMethod(training_features,training_labels,validation_features,validation_labels,params):
    print(params)
    f = open("results_svm_ds1_poly_NEW.txt", "a+")
    classifier = svm.SVC(**params)
    classifier.fit(training_features, training_labels)

    # predict training data
    training_predicted = classifier.predict(training_features);
    accuracy_training = accuracy_score(training_labels, training_predicted) * 100

    # predict validation data
    validation_predicted = classifier.predict(validation_features)
    accuracy_validation = accuracy_score(validation_labels, validation_predicted) * 100
...
class ModelTrainer_SVM_DS1_poly :
    ...
    def experiment_svm(self):
        ...
        grid = [{'kernel': ['poly'],
                 ...
                 'coef0': [0.1,1.0,10.0,100.0]
                 }]

        paramGrid = ParameterGrid(grid)
        #print(len(paramGrid));
        f = open("results_svm_ds1_poly.txt", "a+")
        i=0
    ...
    def experiment_svm_withTimeout(self):
        ...
        i = 0
        for params in paramGrid:
            if (params['C'] < 0.1 and params['coef0'] < 10.0 and params['degree'] < 4 and params['gamman'] < 10.0):
                continue

            p = multiprocessing.Process(target=myMethod, args=(
            self.training_features, self.training_labels, self.validation_features, self.validation_labels, params,))
            p.start()

            p.join(20)

            if p.is_alive():
                print("running... let's kill it...", params)
                # Terminate
                p.terminate()
                p.join()


    def process_svm(self,params):
        classifier = svm.SVC(**params)
        classifier.fit(self.training_features, self.training_labels)
        # predict training data
        training_predicted = classifier.predict(self.training_features);
    ...
    def customParameterSVM(self):
        classifier = svm.SVC(C=0.1, coef0=0.01,degree=4,gamma=100.0,kernel='poly',verbose=True);
        classifier.fit(self.training_features, self.training_labels)
        ...
        validation_predicted = classifier.predict(self.validation_features)
        accuracy_validation = accuracy_score(self.validation_labels, validation_predicted) * 100
        f.write("{'C': 0.1, 'coef0': 0.01, 'degree': 4, 'gamma': 100.0 'kernel': 'poly'}," + str(accuracy_training) + "," + str(accuracy_validation) + "\n")
...
if __name__ == '__main__':
    ...
    #mt.experiment_svm()
    #mt.customParameterSVM()
    mt.experiment_svm_withTimeout()
