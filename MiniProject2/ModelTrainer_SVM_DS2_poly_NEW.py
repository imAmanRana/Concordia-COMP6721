...
def myMethod(training_features,training_labels,validation_features,validation_labels,params):
    f = open("results_svm_ds2_poly_NEW.txt", "a+")
...
class ModelTrainer_SVM_DS2_poly :
    ...
    def experiment_svm(self):
        ...
        for params in paramGrid:
            if(params['C']<0.01 and params['coef0']<10.0 and params['degree']<4 and params['gamman']<100.0):
                continue

            p = multiprocessing.Process(target=myMethod,args=(self.training_features,self.training_labels,self.validation_features,self.validation_labels,params,))
            p.start()
    ...
    def customParameterSVM(self):
        classifier = svm.SVC(C=0.01, coef0=10.0,degree=4,gamma=100.0,kernel='poly',verbose=True);
        classifier.fit(self.training_features, self.training_labels)

        f = open("results_svm_ds2_poly.txt", "a+")
        ...
        validation_predicted = classifier.predict(self.validation_features)
        accuracy_validation = accuracy_score(self.validation_labels, validation_predicted) * 100
        f.write("{'C': 0.01, 'coef0': 10.0, 'degree': 4, 'gamma': 100.0 'kernel': 'poly'}," + str(accuracy_training) + "," + str(accuracy_validation) + "\n")

if __name__ == '__main__':
    mt = ModelTrainer_SVM_DS2_poly()
    mt.dataSet = DATASET1
    mt.fetchData()
